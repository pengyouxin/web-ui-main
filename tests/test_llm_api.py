...
def get_env_value(key, provider, config):
    provider_mappings = {
    ...
    if provider in provider_mappings and key in provider_mappings[provider]:
        return config.get(provider_mappings[provider][key], "")
    return ""
